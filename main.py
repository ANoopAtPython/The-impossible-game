...
class meteorite_left_to_right(pygame.sprite.Sprite):
    def __init__(self):
        super(meteorite_left_to_right , self).__init__()
        self.surf = pygame.Surface((10 , 10))
        self.surf.fill(GREEN)
        self.rect = self.surf.get_rect(
            center = (
...
Add_Meteorite = pygame.USEREVENT + 2
pygame.time.set_timer(Add_Meteorite , 500)
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
            ...
            i,j = player.rect[:2]
            Bullet2 = Bulletin(i , j , 7 , 0)
            Bullet3 = Bulletin(i , j , 7 , 1)
            Bullet4 = Bulletin(i , j , 7 , 2)
            Bullet5 = Bulletin(i , j , 7 , -1)
            Bullet6 = Bulletin(i , j , 7 , -2)
            Bullet7 = Bulletin(i , j , 7 , 3)
            Bullet8 = Bulletin(i , j , 7 , -3)
            Bulletin_Group.add(Bullet6)
            Bulletin_Group.add(Bullet2)
            Bulletin_Group.add(Bullet3)
            Bulletin_Group.add(Bullet4)
            Bulletin_Group.add(Bullet5)
            Bulletin_Group.add(Bullet8)
            Bulletin_Group.add(Bullet7)
        ...
        if event.type == Add_Spread:
            ...
            if boss2.health > 0:
                ...
                spreadshot_group.add(n4)
                spreadshot_group.add(n5)
        if event.type == Add_Meteorite:
            New_Meteorite = meteorite_up()
            meteorite_group.add(New_Meteorite)
            New_Meteorite = meteorite_down()
            meteorite_group.add(New_Meteorite)
            New_Meteorite = meteorite_left_to_right()
            meteorite_group.add(New_Meteorite)
...
if sus == 0:
    if player.health > 0:
        print("You win")
    elif player.health == 0 and boss.health == 0 and boss2.health == 0:
        print("Draw")
    else:
        print("you lose")
else:
